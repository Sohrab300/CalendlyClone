  ...
  return (
    ...
    >
      <div className={cn("flex items-center", isSettings ? "gap-6" : "gap-4")}>
        <button
          className={cn(
            "hover:bg-slate-100 rounded-full transition-colors relative",
            isSettings ? "p-0" : "p-2",
          )}
        >
          {isSettings ? null : ( //<UserPlus className="w-5 h-5 text-[#0b1f3a]" />
            <>
        ...
        <div className="relative" ref={dropdownRef}>
          <div
            ...
            className={cn(
              "flex items-center cursor-pointer hover:bg-slate-50 rounded-lg transition-colors",
              isSettings ? "gap-4 p-0" : "gap-2 p-1",
            )}
          >
            <div
              className={cn(
                "rounded-full flex items-center justify-center font-bold overflow-hidden w-8 h-8 bg-blue-600 text-white text-xs ring-2 ring-blue-100 ring-offset-2",
              )}
            >
              {profile?.avatar_url ? (
                <img
