...
import { X, ChevronDown, Clock, MapPin, Calendar, User, AlertCircle, ChevronUp, Plus, AlignLeft, Zap, DollarSign, Bell, CheckSquare, ChevronLeft, CalendarClock, Settings2, FileText, CircleDollarSign, ExternalLink, Trash2, GripVertical, Info, Lock, Mail, MessageSquare, MoreVertical, ChevronRight, Pencil, RefreshCw, Copy, List, Search, Check, PlusCircle } from 'lucide-react';
...
import { availabilityService, CustomQuestion, ConfirmationLink, DayAvailability, formatTime, TimeSlot, parseTime } from '../../services/availabilityService';
...
const INITIAL_AVAILABILITY: DayAvailability[] = [
  ...
  { day_index: 1, enabled: true, slots: [{ id: '1', start: '9:00am', end: '5:00pm' }] },
  { day_index: 2, enabled: true, slots: [{ id: '2', start: '9:00am', end: '5:00pm' }] },
  { day_index: 3, enabled: true, slots: [{ id: '3', start: '9:00am', end: '5:00pm' }] },
  { day_index: 4, enabled: true, slots: [{ id: '4', start: '9:00am', end: '5:00pm' }] },
  { day_index: 5, enabled: true, slots: [{ id: '5', start: '9:00am', end: '5:00pm' }] },
...
export const CreateEventSidebar: React.FC<CreateEventSidebarProps> = ({ 
  ...
  const [isDurationDropdownOpen, setIsDurationDropdownOpen] = React.useState(false);
  const [isCustomUnitDropdownOpen, setIsCustomUnitDropdownOpen] = React.useState(false);
  ...
  const activeSchedule = schedules.find(s => s.is_active) || (schedules.length > 0 ? schedules[0] : { id: '', name: 'Working hours (default)' });
  const [selectedScheduleId, setSelectedScheduleId] = React.useState(editingEvent?.schedule_id || activeSchedule.id);
  ...
  const [bufferBefore, setBufferBefore] = React.useState(editingEvent?.buffer_before || 0);
  const [bufferAfter, setBufferAfter] = React.useState(editingEvent?.buffer_after || 0);
  const [meetingLimitCount, setMeetingLimitCount] = React.useState<number | null>(editingEvent?.meeting_limit_count ?? null);
  const [meetingLimitPeriod, setMeetingLimitPeriod] = React.useState(editingEvent?.meeting_limit_period || 'day');
  const [showBufferSettings, setShowBufferSettings] = React.useState(!!(editingEvent?.buffer_before || editingEvent?.buffer_after));
  const [showLimitSettings, setShowLimitSettings] = React.useState(editingEvent?.meeting_limit_count !== null && editingEvent?.meeting_limit_count !== undefined);
  const [activeBufferDropdown, setActiveBufferDropdown] = React.useState<'before' | 'after' | null>(null);
  const [isLimitPeriodDropdownOpen, setIsLimitPeriodDropdownOpen] = React.useState(false);

  const [slug, setSlug] = React.useState(editingEvent?.slug || '');
  const [timeIncrement, setTimeIncrement] = React.useState(editingEvent?.time_increment || 30);
  const [timezoneDisplay, setTimezoneDisplay] = React.useState<'detect' | 'lock'>(editingEvent?.timezone_display || 'detect');
  const [isIncrementDropdownOpen, setIsIncrementDropdownOpen] = React.useState(false);

  const [inviteeDetailType, setInviteeDetailType] = React.useState<'name_email' | 'first_last_email'>(editingEvent?.invitee_detail_type || 'name_email');
  const [autofillEnabled, setAutofillEnabled] = React.useState(editingEvent?.autofill_enabled ?? true);
  const [allowGuests, setAllowGuests] = React.useState(editingEvent?.allow_guests || false);
  const [questions, setQuestions] = React.useState<CustomQuestion[]>(editingEvent?.questions || []);

  const [useCustomSchedule, setUseCustomSchedule] = React.useState(editingEvent?.use_custom_schedule || false);
  const [customWeeklyHours, setCustomWeeklyHours] = React.useState<DayAvailability[]>(editingEvent?.custom_weekly_hours || INITIAL_AVAILABILITY);
  const [customDateOverrides, setCustomDateOverrides] = React.useState<any[]>(editingEvent?.custom_date_overrides || []);
  ...
  const formatDateRange = (dates: any[]) => {
    ...
    if (sorted.length === 1) return `${monthNames[start.getMonth()]} ${start.getDate()}`;
    ...
    if (isConsecutive) return `${monthNames[start.getMonth()]} ${start.getDate()} – ${end.getDate()}`;
    return `${monthNames[start.getMonth()]} ${start.getDate()}, ...`;
  };

  const [isDetailTypeDropdownOpen, setIsDetailTypeDropdownOpen] = React.useState(false);
  const [activeQuestionDropdown, setActiveQuestionDropdown] = React.useState<string | null>(null);
  ...
  const [activeInfoPopup, setActiveInfoPopup] = React.useState<'workflows' | 'basic' | null>(null);
  const [activeMoreTooltip, setActiveMoreTooltip] = React.useState<string | null>(null);

  // Confirmation Page State
  const [afterBookingOption, setAfterBookingOption] = React.useState(editingEvent?.confirmation_type === 'redirect' ? 'Redirect to an external site' : 'Display confirmation page');
  const [isAfterBookingDropdownOpen, setIsAfterBookingDropdownOpen] = React.useState(false);
  const [confirmationLinks, setConfirmationLinks] = React.useState<ConfirmationLink[]>(editingEvent?.confirmation_links || [
    { id: 'default', name: 'Schedule another event', url: '', isDefault: true, status: true }
  ]);

  // Availability State
  const [dateRangeKind, setDateRangeKind] = React.useState<'relative' | 'range' | 'indefinite'>(editingEvent?.date_range_kind || 'relative');
  const [dateRangeValue, setDateRangeValue] = React.useState(editingEvent?.date_range_value || 60);
  const [dateRangeType, setDateRangeType] = React.useState<'calendar_days' | 'weekdays'>(editingEvent?.date_range_type || 'calendar_days');
  const [dateRangeStart, setDateRangeStart] = React.useState(editingEvent?.date_range_start || '');
  const [dateRangeEnd, setDateRangeEnd] = React.useState(editingEvent?.date_range_end || '');
  const [tempDateRangeStart, setTempDateRangeStart] = React.useState(editingEvent?.date_range_start || '');
  const [tempDateRangeEnd, setTempDateRangeEnd] = React.useState(editingEvent?.date_range_end || '');
  const [minimumNotice, setMinimumNotice] = React.useState(editingEvent?.minimum_notice || 4);
  ...
  const [isDateRangeTypeDropdownOpen, setIsDateRangeTypeDropdownOpen] = React.useState(false);
  const [isNoticeDropdownOpen, setIsNoticeDropdownOpen] = React.useState(false);
  const [isFixedDatePickerOpen, setIsFixedDatePickerOpen] = React.useState(false);
  ...
  React.useEffect(() => {
    if (selectedScheduleId) {
      availabilityService.getWeeklyHours(selectedScheduleId)
  ...
  const handleSaveDescription = async () => {
    if (editingEvent) {
      try {
        await availabilityService.updateEventType(editingEvent.id, { description });
  ...
  const moreOptionsSections = [
    { icon: AlignLeft, title: 'Description', desc: 'Tell your invitees what this meeting is about' },
    { icon: CalendarClock, title: 'Limits and buffers', desc: 'Buffer times, max limits' },
    { icon: Calendar, title: 'Free/busy rules', desc: 'Allow invitees to book over selected meetings on your connected calendars' },
    { icon: Settings2, title: 'Booking page options', desc: '/30min • 30 min increments • auto time zone' },
    { icon: FileText, title: 'Invitee form', desc: 'Asking for name, email, +1 question' },
    { icon: CircleDollarSign, title: 'Payment', desc: 'Collect payment for your event' },
    { icon: Bell, title: 'Notifications and workflows', desc: 'Calendar invitations' },
    { icon: CheckSquare, title: 'Confirmation page', desc: 'Display confirmation page' },
  ];
      ...
      <div className="w-full">
        ...
        <div className="grid grid-cols-7 mb-4">
          {['SUN', 'MON', 'TUE', 'WED', 'THU', 'FRI', 'SAT'].map(d => (
            <div key={d} className="text-[11px] font-bold text-[#94a3b8] text-center py-2">
              {d}
            </div>
          ))}
        </div>
        <div className="grid grid-cols-7 gap-y-2">
          {days.map((day, i) => {
            const isSelected = modalSelectedDates.find(d => isSameDay(d, day));
              ...
              >
                <div 
                  ...
                  className={cn(
                    ...
                    isTodayDate ? "bg-blue-50 text-blue-600" : (isPastDate ? "text-[#e2e8f0] cursor-not-allowed" : "text-[#334155] hover:bg-[#f8fafc]")
                  )}
                >
                  {format(day, 'd')}
                </div>
        ...
        <div className="flex items-center justify-between mb-6 px-2">
          <button 
            ...
            className={cn(
              ...
              isFirstMonth ? "opacity-0 cursor-default" : "hover:bg-slate-100 text-slate-600"
            )}
          >
            <ChevronLeft className="w-5 h-5" />
          </button>
          ...
          {days.map((day, i) => {
            ...
            return (
              <div 
                ...
                onMouseEnter={() => start && !end && !isPastDate && setHoverDate(day)}
              ...
              >
                ...
                <div className={cn(
                  ...
                  isTodayDate ? "bg-slate-100 text-slate-900" : "text-slate-700 hover:bg-slate-50"
                )}>
                  {format(day, 'd')}
                </div>
              </div>
  ...
  return (
    <>
      ...
      >
        ...
        <div className={cn(
          "flex-1 p-8",
          isFixedDatePickerOpen ? "overflow-hidden" : "overflow-y-auto overflow-x-visible"
        )}>
          <div className="mb-8 relative">
            ...
            <p className="text-sm text-slate-500 mt-1">One-on-One</p>
          </div>
            ...
            {!showMoreOptions ? (
                ...
                <div className="py-6">
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-lg font-bold text-slate-900">Duration</span>
                    ...
                    {expandedSection !== 'Duration' && (
                      <div className="flex items-center gap-2 text-sm text-slate-500">
                        ...
                        <span>
                          {duration === 'Custom' ? `${customValue} ${customUnit}` : duration}
                        </span>
                      </div>
                    )}
                  </div>
                        ...
                        <div className="relative mb-4">
                          <button 
                            ...
                            className={cn(
                              ...
                              isDurationDropdownOpen ? "border-blue-500 ring-2 ring-blue-100" : "border-slate-200 hover:border-slate-300"
                            )}
                          >
                        ...
                        {duration === 'Custom' && (
                          <div className="flex gap-2 mb-4">
                            ...
                            <div className="relative w-32">
                              <button 
                                onClick={() => setIsCustomUnitDropdownOpen(!isCustomUnitDropdownOpen)}
                                className={cn(
                                  "w-full flex items-center justify-between px-4 py-3 border rounded-lg text-sm transition-all bg-white",
                                  isCustomUnitDropdownOpen ? "border-blue-500 ring-2 ring-blue-100" : "border-slate-200 hover:border-slate-300"
                                )}
                              >
                                <span className="text-slate-700">{customUnit}</span>
                  ...
                  >
                    ...
                    {expandedSection !== 'Availability' && (
                      <div className="flex items-center gap-2 text-sm text-slate-500">
                        ...
                        <span>
                          {dateRangeKind === 'indefinite' ? 'Indefinitely' : 
                           dateRangeKind === 'range' ? 'Within date range' :
                    ...
                    {expandedSection === 'Availability' && (
                      ...
                      >
                        ...
                        <div className="space-y-4">
                          <h4 className="text-base font-bold text-slate-900">Date-range</h4>
                          <div className="text-sm text-slate-600 leading-relaxed">
                            ...
                            <span className="relative inline-block">
                              ...
                              >
                                {dateRangeKind === 'indefinite' ? 'indefinitely' : 
                                 dateRangeKind === 'range' ? 'within a date range' :
                                 `${dateRangeValue} ${dateRangeType.replace('_', ' ')}`} 
                                {isDateRangeExpanded ? <ChevronUp className="w-3 h-3" /> : <ChevronDown className="w-3 h-3" />}
                              </button>
                            </span>{' '}
                            ...
                            <span className="relative inline-block">
                              <button 
                                ...
                                className="text-blue-600 font-bold hover:underline inline-flex items-center gap-1"
                              >
                                {minimumNotice} hours <ChevronDown className="w-3 h-3" />
                              </button>
                              <AnimatePresence>
                              ...
                              >
                                <div className="pl-4 border-l-2 border-slate-100 space-y-4 mt-2">
                                  ...
                                  <div className="flex items-start gap-3">
                                    <button 
                                      onClick={() => setDateRangeKind('relative')}
                                      className={cn(
                                        "mt-1 w-5 h-5 rounded-full border flex items-center justify-center transition-colors",
                                        dateRangeKind === 'relative' ? "border-blue-600 bg-blue-600" : "border-slate-300"
                                      )}
                                    >
                                      {dateRangeKind === 'relative' && <div className="w-2 h-2 bg-white rounded-full" />}
                                    </button>
                                    <div className="flex flex-wrap items-center gap-2">
                                      <input 
                                        ...
                                        onChange={(e) => {
                                          ...
                                          setDateRangeValue(val ? parseInt(val) : 0);
                                        }}
                                      ...
                                      <div className="relative">
                                        <button 
                                          onClick={() => setIsDateRangeTypeDropdownOpen(!isDateRangeTypeDropdownOpen)}
                                        ...
                                        >
                                          <span>{dateRangeType.replace('_', ' ')}</span>
                                          <ChevronDown className={cn("w-4 h-4 text-slate-400 transition-transform", isDateRangeTypeDropdownOpen && "rotate-180")} />
                                        </button>
                                        <AnimatePresence>
                                            ...
                                            >
                                              <button
                                                onClick={() => {
                                                  ...
                                                  setIsDateRangeTypeDropdownOpen(false);
                                                }}
                                                className="w-full text-left px-4 py-3 hover:bg-slate-50 transition-colors group"
                                              >
                                                <div className="flex items-center justify-between">
                                                  <span className="text-sm font-bold text-slate-900">calendar days</span>
                                                  {dateRangeType === 'calendar_days' && <div className="w-2 h-3.5 border-b-2 border-r-2 border-blue-600 rotate-45 mb-1" />}
                                                </div>
                                                <p className="text-xs text-slate-500 mt-1">Counts every day on the calendar including days you're unavailable</p>
                                              </button>
                                              <button
                                                onClick={() => {
                                              ...
                                              >
                                                <div className="flex items-center justify-between">
                                                  <span className="text-sm font-bold text-slate-900">weekdays</span>
                                                  {dateRangeType === 'weekdays' && <div className="w-2 h-3.5 border-b-2 border-r-2 border-blue-600 rotate-45 mb-1" />}
                                                </div>
                                                <p className="text-xs text-slate-500 mt-1">Excludes weekends and only counts Mon - Fri</p>
                                              </button>
                                            </motion.div>
                                          )}
                                        </AnimatePresence>
                                      </div>
                                      <span className="text-sm text-slate-600">into the future</span>
                                    </div>
                                  </div>
                                  ...
                                  <div className="flex items-center gap-3">
                                    <button 
                                      ...
                                      className={cn(
                                        ...
                                        dateRangeKind === 'range' ? "border-blue-600 bg-blue-600" : "border-slate-300"
                                      )}
                                    >
                                      {dateRangeKind === 'range' && <div className="w-2 h-2 bg-white rounded-full" />}
                                    </button>
                                    <div className="flex items-center gap-3">
                                      <span className="text-sm text-slate-700">Within a date range</span>
                                      {dateRangeKind === 'range' && (
                                        <div className="relative">
                                          <button
                                            ref={datePickerTriggerRef}
                                            onClick={() => setIsFixedDatePickerOpen(!isFixedDatePickerOpen)}
                                            className={cn(
                                              "h-10 px-4 border rounded-lg flex items-center gap-2 text-sm transition-all bg-white min-w-[220px]",
                                              isFixedDatePickerOpen ? "border-blue-600 ring-4 ring-blue-50" : "border-slate-200 hover:border-slate-300",
                                              (!dateRangeStart || !dateRangeEnd) && "border-red-200 bg-red-50/30"
                                            )}
                                          >
                                            <Calendar className="w-4 h-4 text-slate-400" />
                                            <span className={cn(
                                              "font-medium",
                                              (dateRangeStart && dateRangeEnd) ? "text-slate-900" : "text-slate-400"
                                            )}>
                                  ...
                                  <div className="flex items-center gap-3">
                                    <button 
                                      onClick={() => setDateRangeKind('indefinite')}
                                      className={cn(
                                        "w-5 h-5 rounded-full border flex items-center justify-center transition-colors",
                                        dateRangeKind === 'indefinite' ? "border-blue-600 bg-blue-600" : "border-slate-300"
                                      )}
                                    >
                                      {dateRangeKind === 'indefinite' && <div className="w-2 h-2 bg-white rounded-full" />}
                                    </button>
                                    <span className="text-sm text-slate-700">Indefinitely into the future</span>
                                  </div>
                                </div>
                              </motion.div>
                            )}
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <div className="relative">
                            ...
                            >
                              {useCustomSchedule ? 'Custom' : (schedules.find(s => s.id === selectedScheduleId)?.name || 'Select a schedule')}
                                ...
                                >
                                  {schedules.map((opt) => (
                                    <button
                                      ...
                                      className="w-full flex items-center justify-between px-4 py-3 hover:bg-slate-50 text-sm text-slate-700 transition-colors"
                                    >
                                      <span className={cn(selectedScheduleId === opt.id && !useCustomSchedule && "font-bold text-blue-600")}>{opt.name}</span>
                                      {selectedScheduleId === opt.id && !useCustomSchedule && <Check className="w-4 h-4 text-blue-600" />}
                                    </button>
                                  ))}
                                  ...
                                  <button
                                    onClick={() => {
                                      ...
                                      if (JSON.stringify(customWeeklyHours) === JSON.stringify(INITIAL_AVAILABILITY) && weeklyHours.length > 0) {
                                    ...
                                    className="w-full flex items-center justify-between px-4 py-3 hover:bg-slate-50 text-sm text-slate-700 transition-colors"
                                  >
                                    <span className={cn(useCustomSchedule && "font-bold text-blue-600")}>Custom schedule</span>
                                    {useCustomSchedule && <Check className="w-4 h-4 text-blue-600" />}
                                  </button>
                                </motion.div>
                                )}
                              </AnimatePresence>
                            ...
                            {!useCustomSchedule ? (
                              <>
                                <div className="p-5 flex items-start justify-between border-b border-[#f1f5f9] bg-[#f8fafc]/30">
                                  ...
                                  <button 
                                    onClick={() => onNavigateToAvailability?.(selectedScheduleId)}
                                ...
                                <div className="p-6 space-y-7">
                                  <div className="space-y-4">
                                    <div className="flex items-center gap-2.5 text-[#334155]">
                                      ...
                                      <span className="text-[15px] font-semibold">Weekly hours</span>
                                    </div>

                                    <div className="space-y-3.5">
                                      {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((day, idx) => {
                                        const dayHours = weeklyHours.find(h => h.day_index === idx);
                                        const isEnabled = dayHours?.enabled && dayHours.slots.length > 0;
                                          ...
                                          <div key={idx} className="flex items-center gap-4">
                                            ...
                                            <div className="text-[14px]">
                                              {isEnabled ? (
                                                ...
                                                <span className="text-[#94a3b8] font-medium">Unavailable</span>
                                              )}
                                            </div>
                                          </div>
                                        );
                                  ...
                                  <div className="space-y-4 pt-1">
                                    <div className="flex items-center gap-2.5 text-[#334155]">
                                      ...
                                      <span className="text-[15px] font-semibold">Date-specific hours</span>
                                    </div>
                                    <p className="text-[15px] text-[#334155] pl-[2px] font-medium">None</p>
                                  </div>
                                </div>
                              </>
                            ) : (
                              <div className="p-6 space-y-10">
                                ...
                                <div className="space-y-6">
                                  <div className="flex items-center gap-2 mb-1">
                                    ...
                                    <span className="text-[15px] font-semibold text-[#1e293b]">Weekly hours</span>
                                  </div>
                                  <p className="text-[14px] text-[#64748b]">Set when you are available for meetings</p>
                                        ...
                                        <div key={idx} className="flex items-start gap-4">
                                          <button 
                                            ...
                                            className={cn(
                                              ...
                                              day.enabled ? "bg-[#004dc0] text-white" : "bg-[#f1f5f9] text-[#94a3b8] hover:bg-slate-200"
                                            )}
                                          >
                                            {dayChar}
                                          </button>
                                          ...
                                          <div className="flex-1 space-y-3">
                                            {!day.enabled ? (
                                              <div className="flex items-center gap-4 h-[34px]">
                                                <span className="text-[14px] text-[#94a3b8] font-medium">Unavailable</span>
                                                <button 
                                                  onClick={() => addSlot(idx)}
                                              ...
                                              day.slots.map((slot, slotIndex) => (
                                                <div key={slot.id} className="flex items-center gap-3">
                                                  <div className="relative">
                                                    <button 
                                                      onClick={() => setOpenPicker({ dayIndex: idx, slotId: slot.id, type: 'start' })}
                                                      className={cn(
                                                        ...
                                                        openPicker?.dayIndex === idx && openPicker?.slotId === slot.id && openPicker?.type === 'start' && "border-blue-600 ring-2 ring-blue-100"
                                                      )}
                                                    >
                                                      {slot.start}
                                                    </button>
                                                    <TimePicker 
                                                      value={slot.start}
                                                      isOpen={openPicker?.dayIndex === idx && openPicker?.slotId === slot.id && openPicker?.type === 'start'}
                                                      onClose={() => setOpenPicker(null)}
                                                      onChange={(time) => updateSlot(idx, slot.id, 'start', time)}
                                                    />
                                                  </div>
                                                  ...
                                                  <div className="relative">
                                                    <button 
                                                      onClick={() => setOpenPicker({ dayIndex: idx, slotId: slot.id, type: 'end' })}
                                                      className={cn(
                                                        "w-28 px-3 py-1.5 bg-[#f8fafc] border border-[#e2e8f0] rounded-lg text-sm font-semibold text-[#334155] hover:border-blue-600 transition-all text-center",
                                                        openPicker?.dayIndex === idx && openPicker?.slotId === slot.id && openPicker?.type === 'end' && "border-blue-600 ring-2 ring-blue-100"
                                                      )}
                                                    >
                                                      {slot.end}
                                                    </button>
                                                    <TimePicker 
                                                      value={slot.end}
                                                      isOpen={openPicker?.dayIndex === idx && openPicker?.slotId === slot.id && openPicker?.type === 'end'}
                                                      onClose={() => setOpenPicker(null)}
                                                      onChange={(time) => updateSlot(idx, slot.id, 'end', time)}
                                                      minTime={slot.start}
                                                    />
                                                  </div>
                                ...
                                <div className="text-[14px] text-blue-600 font-bold flex items-center gap-2 cursor-pointer hover:underline">
                                  India, Sri Lanka Time <ChevronDown className="w-4 h-4" />
                                </div>
                                ...
                                <div className="space-y-4 pt-4 border-t border-[#f1f5f9]">
                                  ...
                                  <p className="text-[14px] text-[#64748b] mb-4">Adjust hours for specific days</p>
                                        ...
                                        <div key={override.id} className="flex items-start justify-between group">
                                          <div className="flex-1">
                                            <div className="flex items-center justify-between mb-1.5">
                                              <span className="text-[14px] font-bold text-[#334155]">{formatDateRange(override.dates)}</span>
                                              <div className="flex flex-col items-end">
                                                {override.slots.map(slot => (
                                                  <span key={slot.id} className="text-[13px] text-[#64748b]">{slot.start} – {slot.end}</span>
                                                ))}
                                              </div>
                                            </div>
                                          </div>
              ...
              <div className="py-2">
                ...
                <div className="py-6 border-b border-slate-100">
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-lg font-bold text-slate-900">Description</span>
                      ...
                      <div className="flex items-center gap-2 text-sm text-slate-500">
                        ...
                        <span className="truncate">
                          {description && description !== '<p><br></p>' 
                            ? description.replace(/<[^>]*>/g, '').trim() || 'Tell your invitees what this meeting is about'
                ...
                <div className="py-6 border-b border-slate-100">
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-lg font-bold text-slate-900">Limits and buffers</span>
                      ...
                      <div className="flex items-center gap-2 text-sm text-slate-500">
                        ...
                        <div className="space-y-1">
                          {(bufferBefore > 0 || bufferAfter > 0) && (
                            <p>Buffers: {bufferBefore}m before, {bufferAfter}m after</p>
                          )}
                          {meetingLimitCount !== null ? (
                            <p>Limit: {meetingLimitCount} per {meetingLimitPeriod}</p>
                          ) : (
                            <p>No meeting limit</p>
                          )}
                          {!(bufferBefore > 0 || bufferAfter > 0) && meetingLimitCount === null && (
                  ...
                  <AnimatePresence>
                    {expandedSection === 'Limits and buffers' && (
                      ...
                      >
                        {/* Buffer Times */}
                        <div>
                          <h4 className="text-sm font-bold text-slate-900 mb-1">Buffer times</h4>
                          <p className="text-xs text-slate-500 mb-3">Add buffer time before or after booked events</p>
                          ...
                          {!showBufferSettings ? (
                            ...
                            <div className="space-y-4 border-l-2 border-slate-100 pl-4 py-2">
                              <div>
                                <label className="text-xs text-slate-500 block mb-2">Before event:</label>
                                <div className="relative">
                                  <button 
                                    onClick={() => setActiveBufferDropdown(activeBufferDropdown === 'before' ? null : 'before')}
                                    className={cn(
                                      "w-full flex items-center justify-between px-4 py-2.5 border rounded-lg text-sm transition-all bg-white",
                                      activeBufferDropdown === 'before' ? "border-blue-500 ring-2 ring-blue-100" : "border-slate-200 hover:border-slate-300"
                                    )}
                                  >
                                    <span>{bufferOptions.find(o => o.value === bufferBefore)?.label}</span>
                                    <ChevronDown className={cn("w-4 h-4 text-slate-400 transition-transform", activeBufferDropdown === 'before' && "rotate-180 text-blue-500")} />
                                  </button>
                                  <AnimatePresence>
                              ...
                              <div>
                                <label className="text-xs text-slate-500 block mb-2">After event:</label>
                                <div className="relative">
                                  <button 
                                    onClick={() => setActiveBufferDropdown(activeBufferDropdown === 'after' ? null : 'after')}
                                    className={cn(
                                      "w-full flex items-center justify-between px-4 py-2.5 border rounded-lg text-sm transition-all bg-white",
                                      activeBufferDropdown === 'after' ? "border-blue-500 ring-2 ring-blue-100" : "border-slate-200 hover:border-slate-300"
                                    )}
                                  >
                                    <span>{bufferOptions.find(o => o.value === bufferAfter)?.label}</span>
                                    <ChevronDown className={cn("w-4 h-4 text-slate-400 transition-transform", activeBufferDropdown === 'after' && "rotate-180 text-blue-500")} />
                                  </button>
                                  <AnimatePresence>
                        ...
                        {/* Meeting Limits */}
                        <div>
                          <h4 className="text-sm font-bold text-slate-900 mb-1">Meeting limits</h4>
                          <p className="text-xs text-slate-500 mb-3">Set the maximum meetings allowed for this event type</p>
                            ...
                            <div className="flex items-center gap-3 border-l-2 border-slate-100 pl-4 py-2">
                              <div className="w-20">
                                <input 
                                  ...
                                  onChange={(e) => setMeetingLimitCount(parseInt(e.target.value) || 0)}
                              ...
                              <span className="text-sm text-slate-600">meetings per</span>
                              <div className="relative flex-1">
                                <button 
                                  onClick={() => setIsLimitPeriodDropdownOpen(!isLimitPeriodDropdownOpen)}
                                  className={cn(
                                    "w-full flex items-center justify-between px-4 py-2.5 border rounded-lg text-sm transition-all bg-white",
                                    isLimitPeriodDropdownOpen ? "border-blue-500 ring-2 ring-blue-100" : "border-slate-200 hover:border-slate-300"
                                  )}
                                >
                                  <span>{meetingLimitPeriod}</span>
                                  <ChevronDown className={cn("w-4 h-4 text-slate-400 transition-transform", isLimitPeriodDropdownOpen && "rotate-180 text-blue-500")} />
                                </button>
                                <AnimatePresence>
                ...
                <div className="py-6 border-b border-slate-100">
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-lg font-bold text-slate-900">Free/busy rules</span>
                    ...
                    {expandedSection !== 'Free/busy rules' && (
                      <div className="flex items-center gap-2 text-sm text-slate-500">
                        ...
                        <span className="truncate">Allow invitees to book over selected meetings on your connected calendars</span>
                      </div>
                    )}
                  </div>
                  <AnimatePresence>
                ...
                <div className="py-6 border-b border-slate-100">
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-lg font-bold text-slate-900">Booking page options</span>
                    ...
                    {expandedSection !== 'Booking page options' && (
                      <div className="flex items-center gap-2 text-sm text-slate-500">
                        ...
                        <span className="truncate">/{slug || 'event-slug'} • {timeIncrement} min increments • {timezoneDisplay === 'detect' ? 'auto' : 'locked'} time zone</span>
                      </div>
                    )}
                  </div>
                  <AnimatePresence>
                    {expandedSection === 'Booking page options' && (
                      ...
                      >
                        {/* Event Slug */}
                        <div>
                          <h4 className="text-sm font-bold text-slate-900 mb-1">You can edit the slug for your event</h4>
                          <p className="text-xs text-slate-500 mb-3">{window.location.origin}/{profile?.username || 'user'}/</p>
                          <input 
                            type="text"
                            value={slug}
                        ...
                        <div>
                          <h4 className="text-sm font-bold text-slate-900 mb-1">Start time increments</h4>
                          <p className="text-xs text-slate-500 mb-3">Set the frequency of available time slots for invitees</p>
                          
                          <div className="border-l-2 border-slate-100 pl-4 py-2">
                            <label className="text-xs text-slate-500 block mb-2">Show available start times in increments of...</label>
                            <div className="relative">
                              <button 
                                onClick={() => setIsIncrementDropdownOpen(!isIncrementDropdownOpen)}
                                className={cn(
                                  "w-full flex items-center justify-between px-4 py-2.5 border rounded-lg text-sm transition-all bg-white",
                                  isIncrementDropdownOpen ? "border-blue-500 ring-2 ring-blue-100" : "border-slate-200 hover:border-slate-300"
                                )}
                              >
                                <span>{incrementOptions.find(o => o.value === timeIncrement)?.label}</span>
                                <ChevronDown className={cn("w-4 h-4 text-slate-400 transition-transform", isIncrementDropdownOpen && "rotate-180 text-blue-500")} />
                              </button>
                              <AnimatePresence>
                        ...
                        {/* Time Zone Display */}
                        <div>
                          <h4 className="text-sm font-bold text-slate-900 mb-1">Time zone display</h4>
                          <p className="text-xs text-slate-500 mb-3">Sets how timezone shows on your booking page</p>
                          ...
                          <div className="border-l-2 border-slate-100 pl-4 py-2 space-y-4">
                            <label className="flex items-start gap-3 cursor-pointer group">
                              <div className="relative flex items-center justify-center mt-0.5">
                                ...
                                <div className={cn(
                                  ...
                                  timezoneDisplay === 'detect' ? "border-blue-600 bg-white" : "border-slate-300 group-hover:border-slate-400"
                                )} />
                                {timezoneDisplay === 'detect' && (
                                  <div className="absolute w-2.5 h-2.5 rounded-full bg-blue-600" />
                                )}
                              </div>
                              <span className="text-sm text-slate-700">Automatically detect and show the times in my invitee's time zone</span>
                            </label>
                            ...
                            <label className="flex items-start gap-3 cursor-pointer group">
                              <div className="relative flex items-center justify-center mt-0.5">
                                ...
                                <div className={cn(
                                  "w-5 h-5 rounded-full border-2 transition-all",
                                  timezoneDisplay === 'lock' ? "border-blue-600 bg-white" : "border-slate-300 group-hover:border-slate-400"
                                )} />
                                {timezoneDisplay === 'lock' && (
                                  <div className="absolute w-2.5 h-2.5 rounded-full bg-blue-600" />
                                )}
                              </div>
                              <span className="text-sm text-slate-700">Lock the timezone (best for in-person events)</span>
                            </label>
                          </div>
                        </div>
                      </motion.div>
                ...
                <div className="py-6 border-b border-slate-100">
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-lg font-bold text-slate-900">Invitee form</span>
                    ...
                    {expandedSection !== 'Invitee form' && (
                      <div className="flex items-center gap-2 text-sm text-slate-500">
                        ...
                        <span className="truncate">
                          {inviteeDetailType === 'name_email' ? 'Name, Email' : 'First Name, Last Name, Email'}
                  ...
                  <AnimatePresence>
                    {expandedSection === 'Invitee form' && (
                      ...
                      >
                        {/* Invitee details */}
                        <div>
                          <h4 className="text-sm font-bold text-slate-900 mb-1">Invitee details</h4>
                          <p className="text-xs text-slate-500 mb-3">Collect invitees first name and email, or full name and email</p>
                          
                          <div className="border-l-2 border-slate-100 pl-4 py-2 space-y-4">
                            <div className="relative">
                              <button 
                                onClick={() => setIsDetailTypeDropdownOpen(!isDetailTypeDropdownOpen)}
                                className={cn(
                                  "w-full flex items-center justify-between px-4 py-3 border rounded-lg text-sm transition-all bg-white",
                                  isDetailTypeDropdownOpen ? "border-blue-500 ring-2 ring-blue-100" : "border-slate-200 hover:border-slate-300"
                                )}
                              >
                                <span className="text-slate-700">
                                  {inviteeDetailType === 'name_email' ? 'Name, Email' : 'First Name, Last Name, Email'}
                                </span>
                                <ChevronDown className={cn("w-4 h-4 text-slate-400 transition-transform", isDetailTypeDropdownOpen && "rotate-180 text-blue-500")} />
                              </button>
                              <AnimatePresence>
                                {isDetailTypeDropdownOpen && (
                                  ...
                                  >
                                    {[
                                      ...
                                      { id: 'first_last_email', label: 'First Name, Last Name, Email' }
                                    ].map((opt) => (
                                      <button
                                        key={opt.id}
                                        onClick={() => {
                            ...
                            <label className="flex items-start gap-3 cursor-pointer group">
                              <div className="relative flex items-center justify-center mt-0.5">
                                ...
                                <div className={cn(
                                  ...
                                  autofillEnabled ? "border-blue-600 bg-blue-600" : "border-slate-300 group-hover:border-slate-400 bg-white"
                                )} >
                                  {autofillEnabled && (
                              ...
                              <span className="text-sm text-slate-700">Autofill Invitee Name, Email, and Text Reminder Phone Number (when applicable) from prior bookings</span>
                            </label>
                          </div>
                        </div>
                        ...
                        {/* Invitee guests */}
                        <div>
                          <h4 className="text-sm font-bold text-slate-900 mb-1">Invitee guests</h4>
                          <div className="flex items-center justify-between">
                            <span className="text-sm text-slate-700">Allow invitees to add guests</span>
                            <button 
                        ...
                        <div className="space-y-4">
                          <div className="flex items-center justify-between">
                            <h4 className="text-sm font-bold text-slate-900">Invitee questions</h4>
                          </div>
                          
                          <div className="space-y-8">
                            {questions.map((q, idx) => (
                              <div key={q.id} className="border-l-2 border-slate-100 pl-6 py-2 space-y-6 relative group/question">
                                <div className="flex items-center justify-between">
                                  <div className="flex items-center gap-3">
                                    <GripVertical className="w-4 h-4 text-slate-300 cursor-grab" />
                                    <span className="text-base font-bold text-slate-900">Question {idx + 1}</span>
                                  </div>
                                  <button 
                                  ...
                                  <label className="flex items-center gap-3 cursor-pointer group w-fit">
                                    <div className="relative flex items-center justify-center">
                                      ...
                                      <div className={cn(
                                        ...
                                        q.required ? "border-blue-600 bg-blue-600" : "border-slate-300 group-hover:border-slate-400 bg-white"
                                      )} >
                                        {q.required && (
                                    ...
                                    <span className="text-base font-medium text-slate-700">Required</span>
                                  </label>
                                </div>

                                <div className="space-y-2">
                                  <label className="text-base font-bold text-slate-900 block">Answer Type</label>
                                  <div className="relative">
                                    <button 
                                      onClick={() => setActiveQuestionDropdown(activeQuestionDropdown === q.id ? null : q.id)}
                                      className={cn(
                                        "w-full flex items-center justify-between px-4 py-3.5 border rounded-xl text-base transition-all bg-white font-medium",
                                        activeQuestionDropdown === q.id ? "border-blue-500 ring-2 ring-blue-100" : "border-slate-200 hover:border-slate-300"
                                      )}
                                    >
                                      <span>
                                        {q.type === 'text' && 'One Line'}
                                        {q.type === 'textarea' && 'Multiple Lines'}
                                        {q.type === 'radio' && 'Radio Buttons'}
                                        {q.type === 'checkbox' && 'Checkboxes'}
                                        {q.type === 'select' && 'Dropdown'}
                                        {q.type === 'phone' && 'Phone Number'}
                                      </span>
                                      <ChevronDown className={cn("w-5 h-5 text-slate-400 transition-transform", activeQuestionDropdown === q.id && "rotate-180 text-blue-500")} />
                                    </button>
                                    <AnimatePresence>
                                      {activeQuestionDropdown === q.id && (
                                        ...
                                        >
                                          {[
                                            ...
                                            { id: 'textarea', label: 'Multiple Lines' },
                                            ...
                                            <button
                                              ...
                                              onClick={() => {
                                                const newQuestions = [...questions];
                                                newQuestions[idx].type = opt.id as any;
                                                if (['radio', 'checkbox', 'select'].includes(opt.id) && (!newQuestions[idx].options || newQuestions[idx].options?.length === 0)) {
                                  ...
                                  <div className="space-y-4 pl-4 border-l-2 border-slate-50">
                                    {q.options?.map((option, optIdx) => (
                                      <div key={optIdx} className="space-y-1">
                                          ...
                                          <div className="flex-1 relative">
                                            <input 
                                              ...
                                              className={cn(
                                                ...
                                                !option && "border-red-200 bg-red-50/30"
                                              )}
                                        ...
                                        {!option && (
                                          <p className="text-xs text-red-500 font-medium ml-7">This field is required</p>
                                        )}
                                      </div>
                                    ))}
                                    <button 
                                    ...
                                    <label className="flex items-center gap-3 cursor-pointer group w-fit ml-7 pt-2">
                                      <div className="relative flex items-center justify-center">
                                        <input 
                                          ...
                                          onChange={(e) => {
                                            const newQuestions = [...questions];
                                            newQuestions[idx].includeOther = e.target.checked;
                                            setQuestions(newQuestions);
                                          }}
                                          className="sr-only"
                                        />
                                        <div className={cn(
                                          "w-6 h-6 rounded border-2 transition-all flex items-center justify-center",
                                          q.includeOther ? "border-blue-600 bg-blue-600" : "border-slate-300 group-hover:border-slate-400 bg-white"
                                        )} >
                                          {q.includeOther && (
                                            <div className="w-2 h-3.5 border-b-2 border-r-2 border-white rotate-45 mb-1" />
                                          )}
                                      ...
                                      <span className="text-base font-medium text-slate-700">Include "Other" option</span>
                                    </label>
                                  </div>
                                )}

                                <div className="space-y-2">
                                  <label className="text-base font-bold text-slate-900 block">Status</label>
                                  <div className="flex items-center gap-3">
                                    <span className="text-base font-medium text-slate-500">{q.status ? 'On' : 'Off'}</span>
                                    <button 
                                      onClick={() => {
                                        const newQuestions = [...questions];
                                        newQuestions[idx].status = !newQuestions[idx].status;
                                        setQuestions(newQuestions);
                                      }}
                                      className={cn(
                                        "w-12 h-6 rounded-full transition-colors relative",
                                        q.status ? "bg-blue-600" : "bg-slate-200"
                                      )}
                                    >
                                      <div className={cn(
                ...
                <div className="py-6 border-b border-slate-100">
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-lg font-bold text-slate-900">Payment</span>
                    ...
                    {expandedSection === 'Payment' && (
                      ...
                      >
                        <div className="bg-blue-50/50 rounded-xl p-6 text-center space-y-4 border border-blue-100/50">
                          <div className="space-y-2">
                            <h4 className="text-base font-bold text-slate-900">Connect a payment processor</h4>
                            <p className="text-sm text-slate-600 leading-relaxed max-w-[280px] mx-auto">
                              To collect payments with Calendly, you must first connect either Stripe or PayPal to your Calendly account.
                            </p>
                          </div>
                          <div className="relative inline-block">
                            <button 
                              ...
                              onClick={() => {
                                ...
                                setTimeout(() => setShowPaymentTooltip(false), 2000);
                              }}
                ...
                <div className="py-6 border-b border-slate-100">
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-lg font-bold text-slate-900">Notifications and workflows</span>
                      ...
                      >
                        ...
                        <div className="space-y-3 relative">
                          <div className="flex items-center gap-1.5">
                            <h4 className="text-sm font-bold text-slate-900">Workflows</h4>
                            <button 
                              onClick={() => setActiveInfoPopup(activeInfoPopup === 'workflows' ? null : 'workflows')}
                              className="focus:outline-none"
                            >
                              <Info className={cn(
                                "w-4 h-4 transition-colors",
                                activeInfoPopup === 'workflows' ? "text-blue-900" : "text-slate-400 hover:text-blue-900"
                              )} />
                            </button>
                          </div>
                            ...
                            {activeInfoPopup === 'workflows' && (
                              ...
                              >
                                <p className="text-sm leading-relaxed font-medium">
                                  Workflows are automated pre- and post-meeting emails and texts sent to invitees or hosts. They're fully customizable, and can be applied to multiple events, making them easy to manage and update.
                                </p>
                                <button 
                          ...
                          <div className="bg-blue-50/50 border border-blue-200 rounded-xl p-4 flex gap-3">
                            ...
                            <p className="text-sm text-slate-700 leading-relaxed">
                              Only the owner of the event type can make changes to workflows
                            </p>
                          </div>
                        ...
                        <div className="space-y-3 relative">
                          <div className="flex items-center gap-1.5">
                            <h4 className="text-sm font-bold text-slate-900">Basic notifications</h4>
                            <button 
                              onClick={() => setActiveInfoPopup(activeInfoPopup === 'basic' ? null : 'basic')}
                              className="focus:outline-none"
                            >
                              <Info className={cn(
                                "w-4 h-4 transition-colors",
                                activeInfoPopup === 'basic' ? "text-blue-900" : "text-slate-400 hover:text-blue-900"
                              )} />
                            </button>
                          </div>
                          ...
                          <AnimatePresence>
                            {activeInfoPopup === 'basic' && (
                              ...
                              >
                                <div className="space-y-4">
                                  <p className="text-sm leading-relaxed font-medium">
                                    Basic notifications apply to this event only.
                                  </p>
                                  <p className="text-sm leading-relaxed font-medium">
                                    For more flexible notifications that scale across events, see workflows (above). Workflows include additional templates, and can be sent to invitees and hosts.
                                  </p>
                                </div>
                                <button 
                                  onClick={() => setActiveInfoPopup(null)}
                          ...
                          <div className="border border-slate-200 rounded-xl divide-y divide-slate-100">
                            ...
                            <div className="p-4 flex items-center justify-between hover:bg-slate-50 transition-colors group">
                              <div className="flex items-center gap-4">
                                ...
                                <div>
                                  <p className="text-sm font-bold text-slate-900">Calendar invitation</p>
                                  <p className="text-xs text-slate-500">Immediately after booking</p>
                                </div>
                              </div>
                              <div className="relative">
                                <button 
                                  onClick={() => {
                                    setActiveMoreTooltip(activeMoreTooltip === 'calendar' ? null : 'calendar');
                                    if (activeMoreTooltip !== 'calendar') {
                                      setTimeout(() => setActiveMoreTooltip(null), 2000);
                                    }
                                  }}
                            ...
                            <div className="p-4 flex items-center justify-between hover:bg-slate-50 transition-colors group">
                              ...
                              <div className="flex items-center gap-3">
                                <span className="px-2 py-1 bg-slate-100 text-slate-600 text-[10px] font-bold uppercase rounded tracking-wider">Off</span>
                                <div className="relative">
                                  <button 
                                    onClick={() => {
                                      setActiveMoreTooltip(activeMoreTooltip === 'email_reminders' ? null : 'email_reminders');
                                      if (activeMoreTooltip !== 'email_reminders') {
                                        setTimeout(() => setActiveMoreTooltip(null), 2000);
                                      }
                                    }}
                                    className="p-2 hover:bg-slate-200 rounded-lg transition-colors group/dots"
                            ...
                            <div className="p-4 flex items-center justify-between hover:bg-slate-50 transition-colors group">
                              <div className="flex items-center gap-4">
                                ...
                                <p className="text-sm font-bold text-slate-900">Text reminders</p>
                              </div>
                              <div className="flex items-center gap-3">
                                <span className="px-2 py-1 bg-slate-100 text-slate-600 text-[10px] font-bold uppercase rounded tracking-wider">Off</span>
                                <div className="relative">
                                  <button 
                                    onClick={() => {
                                      setActiveMoreTooltip(activeMoreTooltip === 'text_reminders' ? null : 'text_reminders');
                                      if (activeMoreTooltip !== 'text_reminders') {
                                        setTimeout(() => setActiveMoreTooltip(null), 2000);
                                      }
                                    }}
                                    className="p-2 hover:bg-slate-200 rounded-lg transition-colors group/dots"
                            ...
                            <div className="p-4 flex items-center justify-between hover:bg-slate-50 transition-colors group">
                              <div className="flex items-center gap-4">
                                ...
                                <p className="text-sm font-bold text-slate-900">Email follow-up</p>
                              </div>
                              <div className="flex items-center gap-3">
                                <span className="px-2 py-1 bg-slate-100 text-slate-600 text-[10px] font-bold uppercase rounded tracking-wider">Off</span>
                                <div className="relative">
                                  <button 
                                    onClick={() => {
                                      setActiveMoreTooltip(activeMoreTooltip === 'email_followup' ? null : 'email_followup');
                                      if (activeMoreTooltip !== 'email_followup') {
                                        setTimeout(() => setActiveMoreTooltip(null), 2000);
                                      }
                                    }}
                                    className="p-2 hover:bg-slate-200 rounded-lg transition-colors group/dots"
                ...
                <div className="py-6 border-b border-slate-100">
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-lg font-bold text-slate-900">Confirmation page</span>
                    ...
                    {expandedSection === 'Confirmation page' && (
                      ...
                      >
                        ...
                        <div className="space-y-3">
                          <label className="text-base font-medium text-slate-900">After booking</label>
                          <div className="relative">
                            <button
                              onClick={() => setIsAfterBookingDropdownOpen(!isAfterBookingDropdownOpen)}
                              className={cn(
                                "w-full flex items-center justify-between px-4 py-3 border rounded-xl transition-all text-left",
                                isAfterBookingDropdownOpen ? "border-blue-600 ring-4 ring-blue-50" : "border-slate-200 hover:border-slate-300"
                              )}
                            >
                              <span className="text-slate-900">{afterBookingOption}</span>
                              <ChevronDown className={cn("w-5 h-5 text-blue-600 transition-transform", isAfterBookingDropdownOpen && "rotate-180")} />
                            </button>
                              ...
                              {isAfterBookingDropdownOpen && (
                                ...
                                >
                                  <div 
                                    ...
                                    onClick={() => {
                                      setAfterBookingOption('Display confirmation page');
                                      setIsAfterBookingDropdownOpen(false);
                                    }}
                                  >
                                    <span className="text-slate-900">Display confirmation page</span>
                                    {afterBookingOption === 'Display confirmation page' && <CheckSquare className="w-5 h-5 text-blue-600" />}
                                  </div>
                                  <div 
                                    className="px-4 py-4 flex items-center justify-between hover:bg-slate-50 cursor-pointer group"
                                    onClick={() => {
                                  ...
                                  >
                                    <div className="flex items-center gap-3">
                                      <span className="text-slate-900">Redirect to an external site</span>
                                      <span className="px-2 py-0.5 bg-slate-100 text-slate-600 text-[10px] font-bold uppercase rounded tracking-wider">Coming soon</span>
                                    </div>
                                  </div>
                                </motion.div>
                              )}
                        ...
                        <div className="space-y-6">
                          <label className="text-base font-medium text-slate-900">Links on confirmation page</label>
                          
                          <div className="space-y-6">
                            {confirmationLinks.map((link, index) => (
                              <div key={link.id} className="relative pl-6 border-l-2 border-slate-100 space-y-4">
                                <div className="flex items-center justify-between">
                                  <div className="flex items-center gap-2">
                                    {link.isDefault && <Lock className="w-4 h-4 text-slate-900" />}
                                    <span className="text-sm font-bold text-slate-900">Link name</span>
                                  </div>
                                  {!link.isDefault && (
                                    <button 
                                      onClick={() => setConfirmationLinks(confirmationLinks.filter(l => l.id !== link.id))}
                                ...
                                <div className="flex items-center gap-4">
                                  ...
                                  <div className="flex items-center gap-2">
                                    <span className="text-sm font-medium text-slate-900">{link.status ? 'On' : 'Off'}</span>
                                    <button 
                                      onClick={() => {
                                        const newLinks = [...confirmationLinks];
                                        newLinks[index].status = !newLinks[index].status;
                                        setConfirmationLinks(newLinks);
                                      }}
                                      className={cn(
                                        "w-10 h-5 rounded-full relative transition-colors",
                                        link.status ? "bg-blue-600" : "bg-slate-200"
                                      )}
                                    >
                                      <div className={cn(
                                        "absolute top-0.5 w-4 h-4 bg-white rounded-full shadow-sm transition-all",
                                        link.status ? "left-[22px]" : "left-0.5"
                                      )} />
                                    </button>
                                  </div>
                                </div>
                          ...
                          <button 
                            onClick={() => setConfirmationLinks([...confirmationLinks, { id: Math.random().toString(), name: '', url: '', isDefault: false, status: true }])}
                ...
                {moreOptionsSections.slice(8).map((section, idx) => (
                  <div key={idx} className="py-6 border-b border-slate-100 last:border-0">
        ...
        <div className="p-6 border-t border-slate-100 flex items-center justify-end gap-4 bg-white">
          <div className="flex items-center gap-4">
            ...
            <button 
              onClick={() => {
                if (dateRangeKind === 'range' && (!dateRangeStart || !dateRangeEnd)) {
                ...
                onCreate({
                  ...
                  meeting_limit_count: showLimitSettings ? meetingLimitCount : null,
                  meeting_limit_period: showLimitSettings ? meetingLimitPeriod : null,
                  slug: slug,
                  ...
                  questions: questions,
                  confirmation_type: afterBookingOption === 'Redirect to an external site' ? 'redirect' : 'display',
                  ...
                  custom_weekly_hours: useCustomSchedule ? customWeeklyHours : null,
                  custom_date_overrides: useCustomSchedule ? customDateOverrides : null
                });
              }}
          ...
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="flex-1 p-8 border-r border-[#f1f5f9] overflow-y-auto">
                <div className="mb-8">
                  <h2 className="text-2xl font-bold text-[#1e293b] mb-2">Select the date(s) you want to assign specific hours</h2>
                </div>

                <div className="space-y-8">
                  {/* Search and Quick Selection could be here */}
                  <div className="bg-[#f8fafc] p-1 rounded-xl flex gap-1 mb-8">
                    <button className="flex-1 py-2 text-sm font-bold bg-white rounded-lg shadow-sm text-blue-600">Calendar</button>
                  </div>
              ...
              <div className="w-full md:w-[380px] p-8 bg-[#f8fafc] flex flex-col overflow-y-auto">
                <div className="mb-8">
                  <h3 className="font-bold text-[#1e293b] mb-2">What hours are you available?</h3>
                  {modalSelectedDates.length > 0 ? (
                    <p className="text-sm text-[#64748b]">
                      Applying to {modalSelectedDates.length} {modalSelectedDates.length === 1 ? 'date' : 'dates'}
                    </p>
                  ) : (
                    <p className="text-sm text-[#64748b]">Select dates on the calendar first</p>
                  )}
                </div>
                    ...
                    <div className="space-y-3">
                      {modalSlots.map((slot, sIdx) => (
                        <div key={slot.id} className="flex items-center gap-3">
                          ...
                          <button 
                            onClick={() => setModalSlots(prev => prev.filter(s => s.id !== slot.id))}
                      ...
                      <button 
                        onClick={() => setModalSlots(prev => [
                          ...prev, 
                          { id: Math.random().toString(36).substr(2, 9), start: '9:00am', end: '5:00pm' }
                        ])}
